...
from declarative_data_framework.core.step import BaseStep, register_step, get_step
from declarative_data_framework.core.engine import BaseEngine
from declarative_data_framework.models.pydantic_models import PipelineConfig, SourceConfig, SinkConfig, ColumnSpec, DefaultsConfig, ValidationRuleConfig, TestConfig, ExpectedResultsDataConfig, SourceDataConfig, TransformationStepConfig

from declarative_data_framework.exceptions import ConfigurationError, StepExecutionError, ValidationError

# Import specific step implementations for testing
from declarative_data_framework.engines.pandas.steps.reader import ReadStep as PandasReadStep
from declarative_data_framework.engines.pandas.steps.transformer import TransformStep as PandasTransformStep
from declarative_data_framework.engines.pandas.steps.validator import ValidateStep as PandasValidateStep
from declarative_data_framework.engines.pandas.steps.writer import WriteStep as PandasWriteStep

from declarative_data_framework.engines.spark.steps.reader import ReadStep as SparkReadStep
from declarative_data_framework.engines.spark.steps.tester import TestStep as SparkTestStep
from declarative_data_framework.engines.spark.steps.transformer import TransformStep as SparkTransformStep
from declarative_data_framework.engines.spark.steps.validator import ValidateStep as SparkValidateStep
from declarative_data_framework.engines.spark.steps.writer import WriterStep as SparkWriterStep
...
# Pandas ReadStep
def test_pandas_read_step_csv(mock_engine, dummy_pandas_df):
...
def test_pandas_read_step_unsupported_format(mock_engine):
...
def test_pandas_read_step_missing_source_config(mock_engine):
...
def test_pandas_transform_step_rename_and_eval(mock_engine):
...
# Pandas WriteStep
def test_pandas_write_step_csv(mock_engine, dummy_pandas_df):
    ...
    with patch('pandas.DataFrame.to_csv') as mock_to_csv,
         ...
         patch('os.path.dirname', return_value='./temp_output'):
...
# Spark ReadStep
def test_spark_read_step_path(mock_spark_engine, dummy_spark_df):
...
def test_spark_read_step_gold_pipeline_skips_read(mock_spark_engine):
    mock_spark_engine.config.pipeline_type = "gold"
    step = SparkReadStep(mock_spark_engine)
    result = step.execute()
    assert result is None

def test_spark_read_step_expected_columns_mismatch(mock_spark_engine, dummy_spark_df):
    mock_spark_engine.config.source.path = "/tmp/data.csv"
    mock_spark_engine.config.source.format = "csv"
...
# Spark TransformStep
def test_spark_transform_step_silver(mock_spark_engine, dummy_spark_df):
    mock_spark_engine.config.pipeline_type = "silver"
...
def test_spark_transform_step_gold(mock_spark_engine):
    mock_spark_engine.config.pipeline_type = "gold"
...
# Spark TestStep
def test_spark_test_step_success(mock_spark_engine):
...
def test_spark_test_step_failure(mock_spark_engine):
    mock_spark_engine.config.test.source_data.path = "/tmp/test_source.csv"
    mock_spark_engine.config.test.expected_results_data.table = "expected_table"
...
def test_spark_test_step_missing_test_config(mock_spark_engine):
...
# Spark WriterStep
def test_spark_writer_step_standard_write(mock_spark_engine, dummy_spark_df):
...
def test_spark_writer_step_validation_log_write(mock_spark_engine, dummy_spark_df):
    mock_spark_engine.config.sink.mode = "overwrite"
